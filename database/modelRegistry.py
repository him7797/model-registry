...
dbConnection = connect(user=username, password=password,
...
async def insertModelInformation(id, name,
    ...
    try:
        ...
        insert_mode_file_info = (str(id), str(name), str(categoryId), str(modelId), str(description), str(version), str(createdBy), str(stage), localInference, str(
            liveStatus), json.dumps(modelBasicInfoData), json.dumps(parameterInfoData), json.dumps(metricsInfoData), json.dumps(tags), json.dumps(featuresInfoData))
...
async def getModelByVersion(modelId,modelVersion):
    try:
        cursor = dbConnection.cursor()
        getModelIdFileInfo = """ select * from models where version where modelFile=%s and version like '%(%s)%' """
        get_model_version = (modelId,modelVersion,)
        cursor.execute(getModelIdFileInfo,get_model_version)
        result = cursor.fetchall()
        # cursor.commit()h21
        return result
    except Exception as e:
